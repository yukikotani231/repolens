...
3. **Dynamic Pages**: Server-rendered pages for authenticated content
   - `/dashboard` - Lists user's accessible PRs
   - `/pr/[owner]/[repo]/[number]` - PR detail with diff display
   - These use `getServerSideProps`-like patterns with server components

4. **Static Disabled**: SSG (`output: 'export'`) is disabled because:
...
Main types in `src/types/github.ts`:
...
- `GitHubPRFile` - File changes in PR
- `GitHubReviewComment` - PR review comments

## Future Improvements

- Add comment creation functionality to enable code review
- Implement PR filtering and sorting
- Add dark mode toggle
- Cache optimization strategy
- Custom domain setup
- GitHub Apps (for better permissions and multiple callback URLs)
